...
module.exports = function handler(req, res) {
    ...
    if (!ensureGet(req, res, '/api/generateInvertedColor?input=tomato')) return;

    const rawInput = req.query && req.query.input;
    const input = typeof rawInput === 'string' ? rawInput.trim() : '';

    if (!input) {
        sendJson(res, 400, { error: 'Missing required query parameter "input". Example: /api/generateInvertedColor?input=tomato' });
        return;
    }

    const result = generateInvertedColor(input);
